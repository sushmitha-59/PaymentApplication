...
/*for /wallet/update api , we have implemented to access to users who has authority "user",so we have to check
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        try{
            ...
            if(response.getStatusCode().is2xxSuccessful()){
                ...
                throw new RuntimeException("Got NON 200 response from the users API http://localhost:8080/user/username/ in wallet service");
            }
        } catch (Exception e) {
